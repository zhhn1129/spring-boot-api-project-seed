...
//        System.out.println(list.get(1).getSecondindex());
    ...
    @ResponseBody
    public List<Assessmentresult2019> echartsShow(Model model) {
    ...
    @RequestMapping(value = "/diagramShow")
    @ResponseBody
    public List<Assessmentresult2019> diagramShow(Model model) {
        Condition c=new Condition(Assessmentresult2019.class);
        c.orderBy("id");
        List<Assessmentresult2019> list = (ArrayList<Assessmentresult2019>) assessmentresult2019Service.findByCondition(c);
        return list;
    ...
    @RequestMapping("modifyYga")
    public String modifyYga(Model m,@RequestParam Integer id) {
        Yuegangao yuegangao=yuegangaoService.findById(id);
        m.addAttribute("yuegangao", yuegangao);
        return "modifyYga";
    }

    @RequestMapping("saveYga")
    public String saveYga(Model m,@ModelAttribute Yuegangao yuegangao) {
        yuegangaoService.update(yuegangao);
        Condition c=new Condition(Yuegangao.class);
        c.orderBy("id");
        ArrayList<Yuegangao> list = (ArrayList<Yuegangao>) yuegangaoService.findByCondition(c);
        PageInfo<Yuegangao> pageInfo = new PageInfo<Yuegangao>(list);
        m.addAttribute("pageInfo", pageInfo);
        m.addAttribute("list", list);
        return "yuegangao";
    }
